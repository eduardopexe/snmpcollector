...
###coleta status anterior
      ...
      open(hold,"/hosts_walk/www/hist_cpu_mem_temp/$host");
...
%mem_status=();
         ...
         foreach $item (keys(%mem_nome)){
            ...
            if ($utl_mem>80){
               ...
               $mem_status{$hh[0]}=$mem_status{$hh[0]}.":".$mem_nome{$item}.":alarme:";           
            }
...
#print "$temp_status{$item} | $item \n";
         ...
         $arq=$item;
         open ($arq,">/hosts_walk/www/hist_cpu_mem_temp/$arq");
            ...
            if (length($mem_status{$item})<3){
               ...
               $st_mem="ok;ok;".$mem_io{$item}.";".$mem_proc{$item};

            }
            else{

               $st_mem="alarme;".$mem_status{$item}.";".$mem_io{$item}.";".$mem_proc{$item};

            }

            $cpn="$t;$data_reg;$cpuavg5{$item};$st_cpu;$st_temp;$st_mem;\n";
